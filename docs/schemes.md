...
| `run_lint` / `run_lint_retry` | Clones the branch, detects the stack (`utils/stack_detector.py`), lists the files the branch changed (`git diff --name-only origin/<base>...HEAD`) and lints only those (`scheme_executor/lint_scope.py`): Python runs `ruff check` on the changed `.py` files; Node runs `eslint` on the changed JS/TS files from each file's nearest `package.json` directory (skipping packages with no ESLint dependency or config); Go runs `go vet` on the packages with changed `.go` files; Rust and Java run the stack's whole-project lint, but only when the branch changed files of that language or its build manifest. No relevant changed files passes with a "no changed ... files to lint" message. If the diff against `origin/<base>` cannot be computed the gate fails. In local mode a gate container from the operative image (`HENCHMEN_OPERATIVE_IMAGE`, default `henchmen-operative:local`) clones the branch, computes the diff and runs the commands itself (`scheme_executor/ci_gate.py`); nothing is bind-mounted from the host, and a gate that reports no result, exits non-zero or times out fails. The GitHub token reaches the gate only on its stdin, never its command line or environment; the gate process uses it for the clone and base fetch, resets `origin`'s URL to a token-less form, and then runs every repo-controlled command (install, linter, tests) as uid 65534 with the workspace handed to that user, so that code cannot read the token from the gate process's memory, environment or `.git/config`. A repository whose install step genuinely needs `GITHUB_TOKEN` (for example, GitHub Packages) therefore fails its local gate; the cloud path is unchanged. See SECURITY.md for exactly what is and is not guaranteed. |
| *(Forge CI, not a scheme node)* | In local mode Forge's PR lint uses these same `run_lint` rules (`plan_lint`) in one `ci_gate forge` gate container with the tests. That is deliberately stricter than Forge's cloud lint, which runs `ruff` on changed `.py` files only. See docs/architecture.md, Forge. |
| `fix_lint` | Runs `ruff check --fix` on changed `.py` files or `eslint --fix` on changed JS/TS files (from each file's nearest `package.json`), reverts any fix outside those files, then commits and pushes the in-scope changes. Skipped for stacks without an auto-fixer. No LLM. In local mode (the effective container orchestrator is Docker) the whole node runs in a gate container (`ci_gate fix`): the install and fixers run as uid 65534, every unprivileged process is killed, and the commit and push run as root from a git directory moved out of the workspace, with hooks disabled and the token passed only to the push. |
