...
* flushed to disk (``fsync`` of the file, and of the directory where supported)
  before it is published under its final name;
* never followed through a symbolic link or trusted when another user owns it:
  either is a readable :class:`SecretFileError` rather than a traceback.
...
class SecretFileError(OSError):
    """A secret file exists but must not be used (a symbolic link, not a regular file, or owned by another user).

    An ``OSError`` so every caller that already turns a secrets-directory I/O
    problem into a readable exit or a 503 handles it the same way. The message
    names the file and the fix, never its content.
    """


def check_secret_path(path: Path) -> None:
    """Raise :class:`SecretFileError` unless ``path`` is absent or a regular file this user owns.

    A symbolic link (dangling or not) is refused outright: following it could
    read attacker-chosen bytes as a key or write a secret somewhere else.
    """
    try:
        info = os.lstat(path)
...
def _fsync_directory(directory: Path) -> None:
    """Best-effort ``fsync`` of a directory so a just-published name survives a crash (POSIX only)."""
    if sys.platform == "win32":
        return
