...
A step becomes complete only through :func:`step_succeeded`, which records it
server-side (``SetupStateStore.record_step_complete``); clients cannot write
``completed_steps``.

The response models below (:class:`StepProblem`, :class:`StepSuccess`,
:class:`StepFailure`) are defined exactly once, here (ruling C11). Plans 2B
...
def step_failed(step: SetupStep, problems: list[StepProblem]) -> StepFailure:
    """Build the failure response; nothing is recorded."""
    return StepFailure(step=step, problems=problems)
...
def discover_step_routes() -> dict[SetupStep, StepRoutes]:
    ...
    Fails closed at app build: a module without an ``APIRouter`` named
    ``router``, a router declared with a prefix, or a ``PUBLIC_ROUTE_PATHS``
    entry outside the step's own prefix (or ending in ``/`` or containing
    ``..``) is an error rather than a silently exposed route.
    ...
    *different* module -- that propagates rather than being swallowed as a
    ...
    for step in SetupStep:
        ...
        if not isinstance(router, APIRouter):
            raise TypeError(f"{module_name} must define `router: APIRouter`")
        if router.prefix:
            raise TypeError(f"{module_name}.router must be declared without a prefix; it is mounted at its step path")
        public = frozenset(getattr(module, "PUBLIC_ROUTE_PATHS", frozenset()))
        own_prefix = f"{STEP_ROUTE_PREFIX}/{step.value}/"
        invalid = sorted(
            path for path in public if not path.startswith(own_prefix) or path.endswith("/") or ".." in path
        )
        if invalid:
            raise ValueError(f"{module_name}.PUBLIC_ROUTE_PATHS must stay under {own_prefix}: {invalid}")
        discovered[step] = StepRoutes(router=router, public_paths=public)
    return discovered
