...
def require_aiosqlite() -> Any:
    """Import ``aiosqlite`` lazily, with an actionable error when it is absent."""
    try:
        import aiosqlite as module
    except ImportError as exc:
        raise AiosqliteMissingError() from exc
    return module
...
async def save_run(run: EvalRun, *, db_path: Path | None = None) -> None:
    ...
    async with require_aiosqlite().connect(str(path)) as db:
...
async def get_run(run_id: str, *, db_path: Path | None = None) -> EvalRun | None:
    ...
    path = db_path or _db_path()
    async with require_aiosqlite().connect(str(path)) as db:
        db.row_factory = require_aiosqlite().Row
        await _ensure_schema(db)
